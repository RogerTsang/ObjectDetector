...
int dma_mm2s_sync(unsigned int* dma_virtual_address) {
    volatile unsigned int mm2s_status =  dma_get(dma_virtual_address, MM2S_STATUS_REGISTER);
...
int dma_s2mm_sync(unsigned int* dma_virtual_address) {
    volatile unsigned int s2mm_status = dma_get(dma_virtual_address, S2MM_STATUS_REGISTER);
...
int main() {
    ...
    memset((void *)virtual_destination_address, 0, 32); // Clear destination block
